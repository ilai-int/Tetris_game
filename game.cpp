...
void Game::InitGame(){
    ...
    mPosX = (BOARD_WIDTH/2) + mPieces->get_x_initial_position(mPiece, mRotation);
    mPosY = mPieces->get_y_initial_poisiton(mPiece, mRotation);
...
void Game::DrawPiece(int pX, int pY, int pPiece, int pRotation){
    ...
    for (int i=0; i < PIECE_BLOCKS; i++){
        for (int j=0; j< PIECE_BLOCKS; j++){
            ...
            switch (mPieces->get_block_type(pPiece, pRotation, pX, pY))
            {
            case 1:
                mColor = GREEN;
                break;
            ...
            if (mPieces->get_block_type (pPiece, pRotation, j, i) != 0){
